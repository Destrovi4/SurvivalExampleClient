...
public class ClientConnection implements Runnable {
	...
	protected int port = NetConstants.DEFAULT_PORT;
	...
	public void applyArguments(String[] args) throws Exception {
		...
		try {
			...
			while(!Thread.interrupted()) {
				...
				if(gameView.user.actorTypeList.size() == 0) {
					...
					actorListener.create(command.propertys);
				} else {
					...
					for(ActorView actorView: actorTypeView.actorList) {
						...
						actorListener.process(gameView, actorTypeView, actorView, actorActionWriter);
